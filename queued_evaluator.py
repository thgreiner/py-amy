...
class MultiplexingEvaluator:
    ...
    def run(self, input_queue):
        ...
        features_board = np.zeros(((self.batch_size, 8, 8, 16)), np.int8)
        features_moves = np.zeros((self.batch_size, 4672), np.int8)
        ...
        while True:
            ...
            for i in range(self.batch_size):
                ...
                features_moves[i] = features[1]
                features_non_progress[i] = features[2]

            predictions = model.predict([features_board, features_moves, features_non_progress])
