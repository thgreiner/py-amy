...
static uint64_t blockers_from_index(int index, uint64_t mask) {
    uint64_t blockers = 0;
    int bits = __builtin_popcountll(mask);
    for (int i = 0; i < bits; i++) {
        int bit = poplsb(&mask);
        if (index & (1 << i)) {
            blockers |= set_mask(bit);
        }
    }
    return blockers;
}
...
static uint64_t random_bitboard(void) {
    uint64_t x = 0;
    for (int i = 0; i < 9; i++) {
        int b = rand() % 64;
        x |= (1ULL << b);
...
static void test_attacks(void) {
    for (int i = 0; i < 5000; i++) {
        uint64_t blockers = random_bitboard();
        // print_bitboard(blockers);
        // printf("\n");

        for (int sq = 0; sq < 64; sq++) {
            // uint64_t rook_attacks_magic = rook_attacks(sq, blockers);
            // uint64_t bishop_attacks_magic = bishop_attacks(sq, blockers);

            // assert(rook_attacks_magic == rook_attack_mask(sq, blockers));
            // assert(bishop_attacks_magic == bishop_attack_mask(sq, blockers));

            uint64_t rook_attacks = rook_attack_mask_flood_fill(sq, blockers);
            printf("Rook %d\n", sq);
            print_2bitboards(rook_attacks, blockers);

            assert(rook_attacks == rook_attack_mask(sq, blockers));
            // print_2bitboards(rook_attacks_magic, bishop_attacks_magic);
            // printf("\n");

            uint64_t bishop_attacks =
                bishop_attack_mask_flood_fill(sq, blockers);
            printf("Bishop %d\n", sq);
            print_2bitboards(bishop_attacks, blockers);

            assert(bishop_attacks == bishop_attack_mask(sq, blockers));
        }
    }
}
